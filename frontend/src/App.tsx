...
const PROGRAM_ID: PublicKey | undefined = PROGRAM_ID_STR ? new PublicKey(PROGRAM_ID_STR) : undefined;

// Fallback sans programme
const ESCROW_WALLET = new PublicKey("4ZubhYsJvTLeVtggbtf5qw8oHmXBG4xDrzkZuracGSaa");

// Trésor (doit matcher le state on-chain)
...
// BN → u64 LE Buffer
function u64LeFromNumber(n: number) {
...
// Discriminator Anchor via Web Crypto (navigateur)
async function ixDiscriminator(name: string): Promise<Uint8Array> {
  const ns = `global:${name}`;                 // ex: "global:buy_tickets"
  const bytes = new TextEncoder().encode(ns);  // Uint8Array
  // extrait un vrai ArrayBuffer DOM de la vue
  const ab = bytes.buffer.slice(bytes.byteOffset, bytes.byteOffset + bytes.byteLength);
  const hashBuf = await crypto.subtle.digest("SHA-256", ab as ArrayBuffer);
  return new Uint8Array(hashBuf).subarray(0, 8);
}
...
function TiltCard({
  ...
  const isTouch =
    typeof window !== "undefined" && typeof matchMedia === "function" && matchMedia("(hover: none)").matches;
...
function FeePolicyCard() {
  return (
    <TiltCard>
      ...
      <div className="small muted mt-8">On-chain enforcement (Anchor program + PDA vault).</div>
    </TiltCard>
  );
}
...
function BuyTickets({ connection }: { connection: Connection }) {
  ...
  const [airMsg, setAirMsg] = useState<string | null>(null);
  ...
  const airdrop = useCallback(async () => {
    if (CLUSTER !== "devnet") {
      setAirMsg("Airdrop disponible uniquement sur devnet.");
      return;
    }
    if (!publicKey) {
      setAirMsg("Connecte d’abord ton wallet.");
      return;
    }
    try {
      setAirMsg("Airdrop en cours…");
      const sig = await connection.requestAirdrop(publicKey, 1 * LAMPORTS_PER_SOL);
      await connection.confirmTransaction(sig, "confirmed");
      setAirMsg("✅ Airdrop 1 SOL reçu !");
    } catch (e: any) {
      setAirMsg("❌ " + (e?.message || String(e)));
    }
  ...
  const onBuy = useCallback(async () => {
    ...
    // ---------- Fallback simple transfert (si pas de programme configuré) ----------
    if (!PROGRAM_ID) {
      try {
        setLoading(true);
        ...
        setError(e?.message ?? "Échec de la transaction");
      } finally {
        setLoading(false);
      }
      return;
    ...
    try {
      ...
      // Anchor 0.31: Program(idl, provider, programId?)
      const program = new (anchor as any).Program(
        VOLTNET_IDL as any,     // idl
        PROGRAM_ID!,            // programId (Address-like)
      ...
      const [stateInfo, vaultInfo] = await Promise.all([
        connection.getAccountInfo(statePda, "confirmed"),
        connection.getAccountInfo(vaultPda, "confirmed"),
      ]);
      if (!stateInfo || !vaultInfo) {
        setError("❌ State/Vault PDA introuvable on-chain. Lance l'initialisation du programme (initialize).");
        setLoading(false);
        return;
      }

      // lecture de l’epoch
      const state: any = await (program.account as any)["lotteryState"].fetch(statePda);
      const epoch = new anchor.BN(state.epoch.toString());
      const userTicketsPda = findUserTicketsPda(programId, publicKey, epoch);
      ...
      try {
        ...
        // 2) Fallback "raw ix"
        try {
          const disc = await ixDiscriminator("buy_tickets");
          const data = Buffer.concat([Buffer.from(disc), u64LeFromNumber(count)]);
          const keys = [
          ...
          setError(
            "❌ Paramètre de compte manquant pour l’instruction (vérifie PROGRAM_ID, treasury, state/vault PDA). Détails: " +
            (rawErr?.message || builderErr?.message || "inconnu")
          );
          setLoading(false);
          return;
        }
  ...
  return (
    <TiltCard>
      ...
      <div style={{ display: "grid", gap: 10, gridTemplateColumns: "1fr 1fr" }}>
        ...
        <button onClick={airdrop} className="btn btn-secondary">
          Airdrop (devnet)
        </button>
      </div>
      ...
      {airMsg && <div className="small mt-8">{airMsg}</div>}
      {error && <div className="error mt-12">{error}</div>}
      <div className="small muted mt-12">
        En achetant vous acceptez les règles. Aucune garantie de gains. Les frais sont affichés ci-dessus.
      </div>
    </TiltCard>
  );
}
    ...
    <div className="screen">
      ...
      <div className="container">
        ...
        <div className="subinfo">
          ...
          {PROGRAM_ID ? PROGRAM_ID.toBase58() : "(fallback transfer mode)"}
        </div>
